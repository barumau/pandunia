# Pronúncia e ortografia de pandunia

Pandunia usa o alfabeto latino básico.
Em Pandunia, cada letra sempre representa o mesmo som e nenhuma letra é muda.

O alfabeto da Pandunia é mostrado abaixo junto com os correspondentes símbolos do Alfabeto Fonético Internacional (AFI).

| Pandunia | AFI    | Exemplo    | Descrição |
|:--------:|:------:|:-----------|:----------|
| A        | a      | **a**to    |           |
| B        | b      | **b**olo   |           |
| C        | tʃ ~ ʃ | **tch**au  |           |
| D        | d      | **d**edo   |           |
| E        | e      | **e**vitar | Nunca nasalado ou com som de *i*. |
| F        | f      | **f**ato   |           |
| G        | g      | **g**ota   | Sempre duro, nunca com som de *j*. |
| H        | h ~ x  | ca**rr**o  | O som gutural mudo como em português brasileiro. |
| I        | i      | **i**dade  |           |
| J        | dʒ ~ ʒ | **j**ade   |           |
| K        | kʰ     | **c**asa   |           |
| L        | l      | **l**ata   | Nunca com som de *u*. |
| M        | m      | **m**eia   |           |
| N        | n      | **n**abo   |           |
| P        | pʰ     | **p**elo   |           |
| O        | o      | **o**vo    | Nunca nasalado ou com som de *u*. |
| R        | r ~ ɹ  | ca**r**o   |           |
| S        | s      | **s**apato | Nunca com som de *z*. |
| T        | tʰ     | **t**elha  | Nunca com som de *c*. |
| U        | u      | **u**va    | Nunca nasalado. |
| V        | w ~ ʋ  | **w**eb    | Semivogal. |
| Y        | j      | **i**ate   | Semivogal. |
| Z        | z      | **z**ebra  |            |


## As vogais

Pandunia tem cinco vogais, que são pronunciadas da mesma forma que em espanhol.

    i   u
    e   o
      a

