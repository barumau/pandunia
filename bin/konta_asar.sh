...
fata_leksasli_liste() {
    ...
    sed -i 's/:/-/g' temp/lexasle.txt
    sed -i 's/eng-/eng:/g' temp/lexasle.txt
    sed -i 's/deu-/deu:/g' temp/lexasle.txt
    sed -i 's/fra-/fra:/g' temp/lexasle.txt
    sed -i 's/spa-/spa:/g' temp/lexasle.txt
    sed -i 's/por-/por:/g' temp/lexasle.txt
    sed -i 's/rus-/rus:/g' temp/lexasle.txt
    sed -i 's/ara-/ara:/g' temp/lexasle.txt
    sed -i 's/zho-/zho:/g' temp/lexasle.txt
    sed -i 's/yue-/yue:/g' temp/lexasle.txt
    sed -i 's/jpn-/jpn:/g' temp/lexasle.txt
    sed -i 's/kor-/kor:/g' temp/lexasle.txt
    sed -i 's/vie-/vie:/g' temp/lexasle.txt
    sed -i 's/fas-/fas:/g' temp/lexasle.txt
    sed -i 's/hin-/hin:/g' temp/lexasle.txt
    sed -i 's/ben-/ben:/g' temp/lexasle.txt
    sed -i 's/may-/may:/g' temp/lexasle.txt
    sed -i 's/swa-/swa:/g' temp/lexasle.txt
    sed -i 's/tur-/tur:/g' temp/lexasle.txt
    sed -i 's/hau-/hau:/g' temp/lexasle.txt
    sed -i 's/ful-/ful:/g' temp/lexasle.txt
    sed -i 's/tam-/tam:/g' temp/lexasle.txt
}

konta_asar() {
   filename='temp/lexasle.txt'
   n=1
   asar=0
   while read line; do
      # reading each line
